...
inline double evalFunction(const vector<double> & expected_counts ,
    vector<double> & current_point,
    const vector<double> &lagrange_lambdas,
    int lagrange_sign)
{
	...
	for (int i =0 ;i<num_elements;i++)
	{
        ...
        lagrange_sign*lagrange_lambdas[i]*current_point[i];//+ L0_ALPHA * exp(-current_point[i]/L0_BETA);
		}
		else
		{
...
void inline evalGradient(const vector<double> & expected_counts,
    vector<double> & current_point,
    vector<double> & gradient,
    const vector<double> &langrange_lambdas,
    int lagrange_sign)
{
	int num_elements = expected_counts.size();
	for (int i =0 ;i<num_elements;i++)
	{
		...
		if (current_point[i] != 0.0)
		{
			gradient[i] = -1 *(expected_counts[i] / current_point[i] -lagrange_sign*langrange_lambdas[i];//- L0_ALPHA * exp(-current_point[i]/L0_BETA)/L0_BETA	);
		}
		else
		{
      cerr<<"The current point "<<i<<" was 0"<<endl;
...
void projectedGradientDescentWithArmijoRule(const vector<double> & expected_counts ,
    const vector<double> & current_prob,
    vector<double> & new_prob,
    vector<double> & lagrange_lambdas,
    int langrange_sign)
{
	...
	for (int time = 1; time <= NUM_PGD_ITERATIONS; time++)
	{
		...
		*/
		...
		//	current_beta = 
			...
			double func_value_at_temp_point = evalFunction(expected_counts,
          temp_point,
          lagrange_lambdas,
          lagrange_sign);
